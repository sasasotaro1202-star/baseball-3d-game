...
function encyclopediaCards(){
  return ALL_PLAYERS.map(p=>{
    ...
    return '<div class="player-card compact-player encyclopedia-player '+(owned?'owned':'locked')+(p.limited?' limited-card':'')+'" data-player-id="'+p.id+'">'+
      ...
      (owned?'<button type="button" class="action release-player" data-id="'+p.id+'">放出</button>':'')+
    '</div>';
  }).join('');
}
function renderCollection(){card.innerHTML='<h2>選手名鑑</h2><p>登録 '+save.collection.length+' / '+ALL_PLAYERS.length+' 名　タップで3D選手表示</p><div class="player-grid">'+(encyclopediaCards()||'<p>選手データがありません</p>')+'</div><button class="action back" id="back">ホームへ戻る</button>';$('back').onclick=()=>setMode('home');bindPlayerCards();}
...
window.__gameReady=true;window.__gameVersion="baseball-3d-web-20260920-02";