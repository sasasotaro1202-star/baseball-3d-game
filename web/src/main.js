...
function persist(){saveGame(save);currency.textContent=save.currency.toLocaleString('ja-JP');}
function setMode(mode){
  ...
  else if(mode==='collection')renderCollection(); else if(mode==='settings')renderCard('設定','サウンド・振動・表示品質などを管理します。');
}
...
function renderGacha(){card.innerHTML='<h2>スカウト</h2><p>1回 250コイン</p><div id="reveal" class="reveal">—</div><div class="row"><button class="action" id="pull">スカウトする</button><button class="action" id="back">戻る</button></div>';$('pull').onclick=()=>{const r=pullOnce(save,HISTORIC_PLAYERS);if(r.error){$('reveal').textContent='コイン不足';return}save=r.state;persist();$('reveal').textContent=r.result.rarity+'　'+r.result.player.name;};$('back').onclick=()=>setMode('home');}
...
function pitch(){if(pitchState!=='idle'||match.ended)return;pitchState='pitch';t=0;match=applyOutcome(match,resolvePitch({timing:.5,contact:.7}));updateMatchHUD();}
function swing(){if(pitchState!=='pitch')return;pitchState='hit';t=0;match=applyOutcome(match,resolvePitch({timing:.55,contact:.82}));updateMatchHUD();}
