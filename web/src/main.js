...
function pitch(){if(pitchState!=='idle'||match.ended)return;pitchState='pitch';t=0;void playMatchEvent('pitch','投球モーション');}
function swing(){if(pitchState!=='pitch')return;pitchState='hit';t=0;const outcome=resolvePitch({timing:.55,contact:.82});match=applyOutcome(match,outcome);updateMatchHUD();const type=outcome.type==='HOME_RUN'?'home_run':outcome.type==='DOUBLE'?'double':outcome.type==='TRIPLE'?'triple':outcome.type==='SINGLE'?'single':outcome.type==='OUT'?'out':outcome.type==='STRIKE'?'strikeout':outcome.type==='BALL'?'walk':'play';void playMatchEvent(type,outcome.type);}
