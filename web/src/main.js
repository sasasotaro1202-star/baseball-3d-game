...
function renderGacha(){card.innerHTML='<h2>スカウト</h2><p>目的別に5種類。欲しいタイプを選んでから引きます。</p><div class="gacha-tabs">'+GACHA_BANNERS.map((b,i)=>'<button class="gacha-tab '+(i===0?'selected':'')+'" data-banner="'+b.id+'"><b>'+b.name+'</b><small>'+b.subtitle+'</small></button>').join('')+'</div><div id="banner-info" class="banner-info"></div><div id="reveal" class="reveal">—</div><div class="row"><button class="action" id="pull">このスカウトを引く</button><button class="action" id="back">戻る</button></div>';$('back').onclick=()=>setMode('home');let bannerId=GACHA_BANNERS[0].id;const info=()=>{const b=GACHA_BANNERS.find(x=>x.id===bannerId)||GACHA_BANNERS[0];$('banner-info').textContent=b.kind+' ｜ '+b.rateBonus;};info();card.querySelectorAll('.gacha-tab').forEach(b=>b.onclick=()=>{bannerId=b.dataset.banner;card.querySelectorAll('.gacha-tab').forEach(x=>x.classList.toggle('selected',x===b));info();});$('pull').onclick=()=>{const r=pullOnce(save,ALL_PLAYERS,undefined,bannerId);if(r.error){$('reveal').textContent='コイン不足';return}save=r.state;persist();void playGachaReveal({rarity:r.result.rarity,name:r.result.player.name});$('reveal').textContent=r.result.rarity+'　'+r.result.player.name+(r.duplicate?'　重複→'+r.duplicateReward+'コイン':'');};}
