...
function updateAimUI(){const el=$('aim-cursor');if(!el)return;el.style.left=`${50+aimTarget.x*34}%`;el.style.top=`${50-aimTarget.y*28}%`;el.classList.toggle('active',match.half==='BOTTOM'&&pitchState==='idle');const p=$('trajectory');if(p){p.style.left='50%';p.style.top='50%';p.style.setProperty('--tx',`${aimTarget.x*34}%`);p.style.setProperty('--ty',`${-aimTarget.y*28}%`);p.classList.toggle('active',pitchState==='pitch');}}
function updateZoneUI(){const z=$('strike-zone');if(z)z.classList.toggle('active',match.inning>0);}
function updateAimFromPointer(ev){if(match.half!=='BOTTOM'||pitchState!=='idle')return;const r=$('aim-area')?.getBoundingClientRect();if(!r)return;aimTarget.x=Math.max(-1,Math.min(1,((ev.clientX-r.left)/r.width-.5)*2));aimTarget.y=Math.max(-1,Math.min(1,(.5-(ev.clientY-r.top)/r.height)*2));updateAimUI();}
...
function pitch(){if(pitchState!=='idle'||match.ended)return;if(match.half==='TOP'){pitchState='pitch';t=0;pitchStart=performance.now();swingWindowOpen=true;pitchTarget={x:(Math.random()-.5)*.72,y:(Math.random()-.5)*.72};const pitcherPlayer=ALL_PLAYERS[0];selectedPitch=choosePitch({count:[match.balls,match.strikes],profile:aiProfile(pitcherPlayer,developmentFor(save,pitcherPlayer.id))});window.__lastPitch=selectedPitch;$('pitch-readout').textContent=selectedPitch;void playMatchEvent('pitch',selectedPitch);}else{pitchState='pitch';t=0;pitchStart=performance.now();swingWindowOpen=false;pitchTarget={x:aimTarget.x*.75,y:aimTarget.y*.75};window.__lastPitch=selectedPitch;$('pitch-readout').textContent=selectedPitch;void playMatchEvent('pitch',selectedPitch);}}
