...
function pitch(){if(pitchState!=='idle'||match.ended)return;pitchState='pitch';t=0;$('pitch-readout').textContent='PITCHING…';void playMatchEvent('pitch','投球モーション');}
function swing(){if(pitchState!=='pitch')return;pitchState='hit';t=0;const outcome=resolvePitch({timing:.55,contact:.82});match=applyOutcome(match,outcome);updateMatchHUD();$('pitch-readout').textContent=outcome;const type=outcome==='HOME_RUN'?'home_run':outcome==='DOUBLE'?'double':outcome==='TRIPLE'?'triple':outcome==='SINGLE'?'single':outcome==='OUT'?'out':outcome==='STRIKE'?'strikeout':outcome==='BALL'?'walk':'play';void playMatchEvent(type,outcome);}
