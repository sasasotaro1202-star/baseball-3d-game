...
export function createPlayerModel({uniform=0x163a66,skin=0xc98262,scale=1,profile={}}={}){
  ...
  const build=profile.build||'athletic';
  ...
  g.userData.base={armL,armR,legL,legR};g.userData.profile=profile;
...
export function animatePlayer(model,type,t){
  ...
  const {armL,armR,legL,legR}=model.userData.base,p=Math.max(0,Math.min(1,t)),sig=model.userData.profile?.signature;
  if(type==='pitch'){const w=Math.sin(p*Math.PI);armR.rotation.z=.35-w*(sig==='ohtani'?1.9:1.7);armR.rotation.x=w*(sig==='ruth'?.9:1.1);legL.rotation.x=w*.65;legR.rotation.x=-w*.35}
  else if(type==='swing'){const s=Math.sin(p*Math.PI);const wide=sig==='ruth'?.9:sig==='ichiro'?.62:1;armL.rotation.y=-s*1.4*wide;armR.rotation.y=s*1.4*wide;armL.rotation.z=-.35-s*.7;armR.rotation.z=.35+s*.7;legL.rotation.z=s*.18;legR.rotation.z=-s*.18}
  else if(type==='run'){const s=Math.sin(p*Math.PI*2);const speed=sig==='ichiro'||sig==='cobb'?1.15:1;legL.rotation.x=s*.8*speed;legR.rotation.x=-s*.8*speed;armL.rotation.x=-s*.55*speed;armR.rotation.x=s*.55*speed}
