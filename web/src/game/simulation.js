...
function walk(s){const live=s.runners.filter(r=>r.status==='LIVE');if(live.some(r=>r.base===0)&&live.some(r=>r.base===1)&&live.some(r=>r.base===2))scoreRuns(s,1);if(live.some(r=>r.base===1)&&live.some(r=>r.base===0)){const r=live.find(x=>x.base===1);if(r)r.base=2;}if(live.some(r=>r.base===0)){const r=live.find(x=>x.base===0);if(r)r.base=1;}addBatter(s);const b=s.runners.find(r=>r.base===-1);if(b)b.base=0;removeScored(s);}
